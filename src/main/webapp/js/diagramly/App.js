...
App.prototype.showSplash = function (force) {
    ...
    if (this.editor.isChromelessView()) {
    ...
    } else if (urlParams["new"]) {
        this.createFile(this.defaultFilename + (EditorUi.isElectronApp ? '.drawio' : ''),
            null, null, null, null, null, null, urlParams['local'] != '1');
        var fileData = this.getCurrentFile()
        // 创建FormData对象
        ...
        // 发送POST请求
        fetch("http://127.0.0.1:8000/draw/upload/",{
            method:"post",
            body:formData,
        ...
        fetch('http://127.0.0.1:8000/draw/detail/' + urlParams["id"] + '/')
            .then(response => response.json())
            .then(fileInfo => {
                // 解析后的数据
                fetch('http://127.0.0.1:8000/draw/download/' + fileInfo.file_path
                    + '/')
